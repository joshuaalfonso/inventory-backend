...
export const getIncomingController = async (c) => {
    return c.json({
        success: true
    });
};
export const createIncomingController = async (c) => {
    const conn = await pool.getConnection();
    try {
        const { purchase_order_id, incoming_date, incoming_item } = c.req.valid('json');
