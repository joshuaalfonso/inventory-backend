...
export const getPuchaseOrderController = async (c) => {
    try {
        const [poRows] = await pool.query(`
            ...
            `);
        const formatted = poRows.map((item) => ({
            ...item,
...
export const getSinglePurchaseOrderController = async (c) => {
    try {
...
export const getPaginatedPurchaseOrdersController = async (c) => {
    try {
...
export const createPurchaseOrderController = async (c) => {
    const conn = await pool.getConnection();
    try {
        const { purchase_request_number, purchase_order_number, purchase_order_date, supplier_id, purchase_order_item } = c.req.valid('json');
        await conn.beginTransaction();
        const [poResult] = await conn.query(`INSERT INTO purchase_order (
                purchase_request_number,
                purchase_order_number,
                purchase_order_date,
                supplier_id
        ...
        if (purchase_order_item && purchase_order_item.length > 0) {
            const values = purchase_order_item.map((item) => [
                purchaseOrderId,
                item.item_id,
                item.employee_id,
...
export const updatePurchaseOrderController = async (c) => {
    const conn = await pool.getConnection();
    try {
        const { purchase_order_id, purchase_request_number, purchase_order_number, purchase_order_date, supplier_id, purchase_order_item } = c.req.valid('json');
        await conn.beginTransaction();
        await conn.query(`
                UPDATE 
                    purchase_order 
